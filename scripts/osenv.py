...
def ensure_msvc_env() -> bool:
    ...
    try:
        ...
        if not vswhere.exists():
            print(f"[msvc] vswhere not found at {vswhere}", file=sys.stderr)
            return False
        ...
        # activate its C++ env script. -prerelease so Preview installs are found too.
        lines = subprocess.run([str(vswhere), "-latest", "-prerelease", "-products", "*",
                                "-property", "installationPath"],
                               capture_output=True, text=True, errors="ignore").stdout.strip().splitlines()
        install = lines[0].strip() if lines else ""
        print(f"[msvc] vswhere installationPath='{install}'", file=sys.stderr)
        if not install:
            return False
        root = Path(install)
        ...
        print(f"[msvc] after activate: cl={shutil.which('cl')} ninja={shutil.which('ninja')}", file=sys.stderr)
        return bool(shutil.which("cl"))
    except (OSError, subprocess.SubprocessError) as e:
        print(f"[msvc] activation failed: {e}", file=sys.stderr)
        return False
