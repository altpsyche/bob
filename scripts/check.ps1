...
. "$PSScriptRoot\_platform.ps1"   # NC1 seam: Get-VenvExe (OS-aware, honors BOB_FORCE_OS). No heavy deps.
...
$venvPy = if ($env:BOB_PYTHON) { $env:BOB_PYTHON } else { Get-VenvExe -Venv 'venv-litellm' -Exe 'python' }
