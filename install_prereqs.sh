...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  elif command -v rpm-ostree >/dev/null 2>&1; then $SUDO rpm-ostree install --idempotent --allow-inactive python3
  else
    log "No supported package manager (apt/dnf/pacman/zypper/rpm-ostree). Install python3 manually — see docs/MANUAL-INSTALL.md."
    exit 1
  fi
fi
if ! command -v python3 >/dev/null 2>&1; then
