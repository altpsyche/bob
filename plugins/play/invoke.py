#!/usr/bin/env python3
"""bob play: open music in Spotify or YouTube Music.
...
The logic lives here as importable functions (music_play, music_stop); tool.py exposes them to the
agent and main() is the `bob play` CLI.
"""
import argparse
import os
import sys
...
for _p in (REPO / "scripts", REPO / "scripts" / "tools"):   # bob_core._port + osenv on sys.path
...
def _find_youtube_url(query: str) -> str | None:
    ...
    try:
        ...
        for result in r.json().get("results", []):
            url = result.get("url", "")
            if url.startswith(("https://", "http://")) and "youtube.com/watch" in url:
                return url
...
def _play_stream(url: str) -> bool:
    ...
    if not (mpv and ytdlp):
        return False
    import subprocess
    _stop_players()   # one player at a time: do not stack windows/audio on a new request
    ...
    argv += ["--", url]   # the URL is data, never an mpv option
    try:
...
def _open(uri: str) -> bool:
    ...
    import osenv   # scripts/ is on sys.path (module import above)
    return osenv.open_url(uri)


# ---------------------------------------------------------------------------
# Entry points (the tool and the CLI both call these)
# ---------------------------------------------------------------------------
...
def music_play(query: str, platform: str = "auto") -> str:
    query = query.strip()
    if not query:
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="bob play", description="Play music via Spotify or YouTube")
    p.add_argument("query", nargs="*", help="Artist, song, album, or playlist")
    g = p.add_mutually_exclusive_group()
    g.add_argument("--youtube", action="store_true", help="Force YouTube")
    g.add_argument("--spotify", action="store_true", help="Force Spotify")
    args = p.parse_args(argv)

    query = " ".join(args.query).strip()
    if not query:
        p.print_usage(sys.stderr)
        print("Example: bob play lofi hip hop", file=sys.stderr)
        return 1
    try:
        from bob_core import load_config
        configure(load_config())
    except Exception:   # noqa: BLE001 (playback works without a resolved config)
        configure({})
    platform = "youtube" if args.youtube else "spotify" if args.spotify else "auto"
    print(music_play(query, platform))
    return 0
